...
const applyDiscount = (base, discount) => {
  ...
  return {
    amount,
    final: parseFloat((base - amount).toFixed(2)),
  };
};
...
const createOrder = async (req, res) => {
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.isCustom) {
        if (!item.name || !item.name.trim()) {
          return res.status(400).json({
            success: false,
            message: `Custom item ${i + 1}: name is required`,
          });
        }
        ...
        if (!Number.isFinite(price) || price < 0) {
          return res.status(400).json({
            success: false,
            message: `Custom item ${i + 1}: invalid price`,
          });
        }
        ...
        if (!Number.isInteger(qty) || qty < 1) {
          return res.status(400).json({
            success: false,
            message: `Custom item ${i + 1}: invalid quantity`,
          });
        }
      ...
      if (!item.productId) {
        return res.status(400).json({
          success: false,
          message: `Item ${i + 1}: productId required for non-custom items`,
        });
      }
      ...
      if (item.price !== undefined) {
        ...
        if (!Number.isFinite(override) || override < 0) {
          return res.status(400).json({
            success: false,
            message: `Invalid price override for "${product.name}"`,
          });
        }
    ...
    // ── Phase 2: Split payment support ───────────────────────────────────
    // payments array: [{ method, amount, reference }]
    // Falls back to legacy single-cash if payments not provided
    ...
    if (Array.isArray(req.body.payments) && req.body.payments.length > 0) {
      ...
      for (const p of req.body.payments) {
        ...
        if (!Number.isFinite(amt) || amt < 0) {
          return res.status(400).json({
            success: false,
            message: `Invalid payment amount for ${p.method}`,
          });
        }

        if (!["cash", "gcash", "maya", "card", "other"].includes(p.method)) {
          return res.status(400).json({
            success: false,
            message: `Unknown payment method: ${p.method}`,
          });
        }
        ...
        payments.push({
          ...
          reference: p.reference?.trim().slice(0, 60) || "",
        });
      }
      ...
      if (totalPaid < total) {
        return res.status(400).json({
          success: false,
          message: `Total paid ₱${totalPaid.toFixed(2)} is less than order total ₱${total.toFixed(2)}`,
          field: "payments",
        });
      }

      // Cash portion for drawer tracking
      ...
      change = parseFloat((totalPaid - total).toFixed(2));

    } else {

      // ── Legacy single-cash path ────────────────────────────────────────
      const cashValidation = validateCash(cash);

      if (!cashValidation.valid) {
        return res.status(400).json({
          success: false,
          message: cashValidation.reason,
          field: "cash",
        });
      }
