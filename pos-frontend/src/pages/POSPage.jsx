...
// ── Cash validation (mirrors backend logic) ───────────────────────────────────
const validateCashInput = (rawCash, total) => {
  ...
  if (num < total) {
    return { valid: false, message: `Cash ₱${num.toFixed(2)} is less than total ₱${total.toFixed(2)}` }
  }
...
export default function POSPage() {
  ...
  const { cart, total, itemCount, addToCart, removeFromCart, incrementQty, decrementQty, overridePrice, clearCart } = useCart()
  ...
  // Debounce search — wait 300ms before filtering (⚡ performance)
  ...
  const loadProducts = useCallback(async () => {
    try {
      ...
      toast('Failed to load products. Is the backend running?', 'error')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  // Filter products — memoized so it only recalculates when deps change (⚡ performance)
  ...
  // Quick items — memoized
  const quickProducts = useMemo(
    () => products.filter((p) => quickIds.includes(p._id)),
    [products, quickIds]
  )

  // Auto-focus search on any keypress (barcode scanner support)
  useEffect(() => {
    const handler = (e) => {
      // e.target is the element that received the keydown event — more reliable than
      // document.activeElement in edge cases (focus changing mid-event cycle)
      const active = e.target
      const tag    = active?.tagName

      // Only redirect to search when truly nothing interactive is focused.
      // Blocks: any input, textarea, select, button (e.g. cash preset buttons),
      //         and any element with contentEditable or a custom role.
  ...
  // ── Validate cash on every change ──────────────────────────────────────────
  const cashValidation = useMemo(() => {
    if (cash === '') return { valid: null, message: '' } // Empty = no error shown yet
    return validateCashInput(cash, total)
  }, [cash, total])

  const change = useMemo(() => {
    const num = Number(cash)
    return Number.isFinite(num) && num >= total ? parseFloat((num - total).toFixed(2)) : null
  }, [cash, total])
  ...
  const isCheckoutDisabled = useMemo(() =>
    ...
    cart.length === 0 ||
    cash === '' ||
    cashValidation.valid === false,
    [checkoutLoading, cart.length, cash, cashValidation.valid]
  )
  ...
  const handleCheckout = useCallback(async () => {
    ...
    try {
      const payload = {
        items: cart.map((item) => ({
          productId: item._id,
          price:     item.price,     // Send price override to backend
          quantity:  item.quantity,
        })),
        cash: Number(cash),          // Always send as a Number, never string
      }
      ...
      if (autoPrint) {
        setTimeout(() => printReceipt(order, { paperWidth, autoPrint: true }), 500)
      }
    } catch (err) {
      // Backend validation errors (cash, stock, etc.)
      if (err.message?.toLowerCase().includes('cash')) {
  ...
  }, [cart, cash, total, clearCart, loadProducts, autoPrint, paperWidth])
    ...
    <div className="flex flex-col h-full bg-surface-800">
      <div className="px-4 py-3 border-b border-surface-700 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          {itemCount > 0 && (
            <span className="px-2 py-0.5 bg-amber-500 text-slate-900 text-xs font-bold rounded-full">{itemCount}</span>
          )}
        </div>
        {cart.length > 0 && (
      ...
      {cart.length > 0 && (
        <div className="shrink-0 border-t border-surface-700 p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-display font-bold text-2xl text-amber-400 font-mono">₱{total.toFixed(2)}</span>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-1.5 mt-2 flex-wrap">
              ...
              <button
                onClick={() => { setCash(total.toFixed(2)); setCashError('') }}
          ...
          {change !== null && (
            <div className="bg-green-900/40 border border-green-700/50 rounded-xl px-3 py-2 flex items-center justify-between animate-slide-in">
              ...
              <span className="font-mono font-bold text-lg text-green-400">₱{change.toFixed(2)}</span>
            </div>
          )}
  ...
  ), [cart, total, itemCount, cash, cashError, cashValidation, change, isCheckoutDisabled, checkoutLoading, autoPrint, paperWidth, handleCheckout, clearCart, incrementQty, decrementQty, removeFromCart, overridePrice])
    ...
    <div className="flex h-full overflow-hidden">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
              {quickProducts.map((p) => (
                <button key={p._id} onClick={() => addToCart(p)} disabled={p.stock === 0}
                  ...
                  <span className="text-amber-400 font-mono text-sm font-bold mt-0.5">₱{p.price.toFixed(2)}</span>
                </button>
              ))}
            </div>
          </div>
              ...
              {filtered.map((product) => (
                <div key={product._id} className="relative group/card">
                  ...
                  <button
                    ...
                    title={quickIds.includes(product._id) ? 'Remove quick' : 'Add to quick items'}
      ...
      {itemCount > 0 && (
        <button
          ...
          className="lg:hidden fixed bottom-5 right-5 z-30 btn-primary px-5 py-3 rounded-2xl shadow-2xl shadow-amber-500/20"
        >
          ...
          <span className="font-bold">{itemCount} · ₱{total.toFixed(2)}</span>
        </button>
      )}
          ...
          <div className="space-y-4">
            <div className="bg-surface-900 rounded-xl p-4 font-mono text-sm space-y-2">
              ...
              {lastOrder.items.map((item, i) => (
                <div key={i} className="flex justify-between text-xs">
                  ...
                  <span className="text-slate-200">₱{item.subtotal.toFixed(2)}</span>
                </div>
              ))}
              <div className="border-t border-surface-700 pt-2 mt-2 space-y-1">
                ...
                {lastOrder.cash > 0 && (
                  <>
                    <div className="flex justify-between text-slate-400 text-xs"><span>Cash</span><span>₱{lastOrder.cash.toFixed(2)}</span></div>
                    <div className="flex justify-between text-green-400 text-xs"><span>Change</span><span>₱{lastOrder.change.toFixed(2)}</span></div>
                  </>
                )}
              </div>
            </div>
            ...
            {lastOrder.change > 0 && (
              <div className="bg-green-900/40 border border-green-700/50 rounded-xl p-4 text-center">
                ...
                <p className="text-green-300 font-mono font-bold text-3xl">₱{lastOrder.change.toFixed(2)}</p>
              </div>
            )}

            <div className="flex gap-2">
              <button
                onClick={() => printReceipt(lastOrder, { paperWidth, autoPrint: true })}
                className="btn btn-secondary flex-1 py-2.5 gap-2"
              >
              ...
              <button onClick={() => setShowReceipt(false)} className="btn-primary flex-1 py-2.5 font-bold rounded-xl">
                New Transaction
              </button>
            </div>
          </div>
