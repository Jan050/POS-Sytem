...
export default function POSPage() {
  ...
  // ── Split Payment ────────────────────────────────────────────────────────
  const [paymentMode, setPaymentMode]         = useState('cash') // 'cash' | 'split'
  ...
  const isCheckoutDisabled =
    checkoutLoading ||
    cart.length === 0 ||
    (paymentMode === 'cash' && (cash === '' || !cashValidation.valid)) ||
  ...
  useEffect(() => { loadProducts(); refreshHeldCount() }, [loadProducts, refreshHeldCount])
  ...
  }, [cart, discountType, discountValue, cartSubtotal, clearCart])
  ...
  const resumeHeldOrder = useCallback((order) => {
    ...
    order.items.forEach((item) => {
      for (let i = 0; i < item.quantity; i++) {
        addToCart({
          _id:      item._id || `custom_${Date.now()}_${Math.random()}`,
          name:     item.name,
          price:    item.price,
...
const handleCheckout = useCallback(async () => {
  ...
  const CartPanel = useCallback(() => (
      ...
      <div className="px-4 py-3 border-b border-surface-700 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="relative text-xs px-2 py-1 rounded-lg bg-surface-600 text-slate-300 hover:bg-surface-500 transition-colors"
          >
            📋
            {heldCount > 0 && (
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-600 py-10">
            <svg className="w-12 h-12 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
                d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-1.4 5.6M17 18a1 1 0 100 2 1 1 0 000-2zm-8 0a1 1 0 100 2 1 1 0 000-2z" />
            </svg>
        ...
        <div className="shrink-0 border-t border-surface-700 p-4 space-y-3">
          ...
          <div className="flex bg-surface-700 rounded-xl p-1 gap-0.5">
            {[
              ...
              <button key={key}
                onClick={() => { setPaymentMode(key); setCashError('') }}
                className={`flex-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all
                  ${paymentMode === key ? 'bg-amber-500 text-slate-900 shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
              >
                {label}
              </button>
            ))}
          ...
          {paymentMode === 'cash' ? (
            <div>
              ...
              <div className={`relative rounded-lg ${cashError ? 'ring-1 ring-red-500' : ''}`}>
                ...
                <input
                  type="number" min="0" step="1" value={cash}
            ...
            <SplitPaymentPanel
              total={total}
              onChange={(payments, valid) => { setSplitPayments(payments); setSplitValid(valid) }}
            />
          )}
          ...
          <button onClick={handleCheckout} disabled={isCheckoutDisabled}
            ...
            ) : paymentMode === 'cash' && cash === '' ? 'Enter cash to checkout'
              : paymentMode === 'cash' && !cashValidation.valid ? 'Fix cash amount'
    ...
    checkoutLoading, autoPrint, paperWidth, paymentMode, splitValid, heldCount,
    handleCheckout, clearCart, incrementQty, decrementQty, removeFromCart, overridePrice,
  ])
    ...
    <div className="flex h-full overflow-hidden">
              ...
              {[
                ...
                <button key={key}
                  ...
                  className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-all
                    ${priceLevel === key ? 'bg-amber-500 text-slate-900' : 'text-slate-400 hover:text-slate-200'}`}
                >
                  {label}
                </button>
              ))}
        ...
        {quickProducts.length > 0 && (
          <div className="px-3 pt-2 shrink-0">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {quickProducts.map((p) => (
                <button key={p._id}
                  onClick={() => addToCart(p, priceLevel === 'wholesale' && p.wholesalePrice != null ? p.wholesalePrice : null)}
                  disabled={p.stock === 0}
                  ...
                  <span className="text-amber-400 font-mono text-sm font-bold mt-0.5">
                    {formatPeso(priceLevel === 'wholesale' && p.wholesalePrice != null ? p.wholesalePrice : p.price)}
                  </span>
                </button>
              ))}
            </div>
              ...
              {filtered.map((product) => (
                <div key={product._id} className="relative group/card">
                  <ProductCard
                    product={product}
                    onAdd={(p) =>
                      addToCart(p, priceLevel === 'wholesale' && p.wholesalePrice != null ? p.wholesalePrice : null)
                    }
      ...
      {itemCount > 0 && (
        <button onClick={() => setShowCartMobile(true)}
          className="lg:hidden fixed bottom-5 right-5 z-30 btn-primary px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-2">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13l-1.4 5.6M17 18a1 1 0 100 2 1 1 0 000-2zm-8 0a1 1 0 100 2 1 1 0 000-2z" />
          </svg>
          ...
          <div className="space-y-4">
            <div className="bg-surface-900 rounded-xl p-4 font-mono text-sm space-y-2">
              ...
              <div className="border-t border-surface-700 pt-2 mt-2 space-y-1">
                ...
                {lastOrder.payments?.length > 0 && (
                  lastOrder.payments.map((p, i) => (
                    <div key={i} className="flex justify-between text-slate-400 text-xs capitalize">
                      <span>{p.method}</span><span>{formatPeso(p.amount)}</span>
                    </div>
                  ))
                )}
              </div>
